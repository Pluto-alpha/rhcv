...
const visitorPass = asyncHandler(async (req, res) => {
    try {
        ...
        const url = `https://${process.env.IP}:5001/files/${fileName}`; // replace with http://10.130.8.102:8080
...
const updateVisitorImg = asyncHandler(async (req, res) => {
    try {
        ...
        const filePath = `https://${process.env.IP}:5001/files/uploads/${file.filename}`;//replace with http://10.130.8.102:8080 
...
const findCaseDetail = asyncHandler(async (req, res) => {
    try {
        ...
        const cases = await Case.findOne({ case_no });
        if (!cases) {
